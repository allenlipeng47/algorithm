...
public class PostOrderNonTraverse {
	...
	public static void PostTraverse(Tree tree){
		...
		while(!s.isEmpty()){
			curr = s.get(s.size()-1);
			if(prev==null||prev.left==curr||prev.right==curr||(curr.left != prev)&&(curr.right != prev)){
				//新进来的
	...
	public static void PostTraverse2(Tree tree){
		if(tree==null){
			return;
		}
		ArrayList<Tree> s = new ArrayList<Tree>();
	...
	public static void InTraverse(Tree tree){
		if(tree==null){
			return;
		}
		ArrayList<Tree> s = new ArrayList<Tree>();
